...
void printInstruction(Instruction *i) {
    ...
    switch (o.type) {
        case iType:
            printf("%-4s %02i=0x%08ux, %02i=0x%08ux, 0x%04x\n", o.name, i->i.rt, registers[i->i.rt], i->i.rs,
                   registers[i->i.rs], i->i.immediate);
            break;
        case jType:
            printf("%-4s 0x%08x\n", o.name, i->j.address);
            break;
        case rType:
            ...
            printf("%-4s %02i=0x%08ux, %02i=0x%08ux, %02i=0x%08ux, 0x%04x\n", f.name, i->r.rd, registers[i->r.rd],
                   i->r.rs, registers[i->r.rs], i->r.rt, registers[i->r.rt], i->r.shamt);
            break;
        case specialType:
            printf("%-4s\n", o.name);
            break;
...
/* ======================================================================= */
/* Initialize and run */
void assignOperation(unsigned short opCode, const char name[OP_NAME_LENGTH + 1], InstructionType type,
